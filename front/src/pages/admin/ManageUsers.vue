...
async function loadUsers(page = 1, perPage = 10, q = '') {
  ...
  try {
      ...
      // AQUÍ ESTABA EL ERROR: Dejamos que Axios use la URL base global
      const resPromise = axios.get('/auth/users', { params, signal: currentAbort.controller.signal, timeout: 8000 })
...
async function loadRoles() {
  try {
    ...
    const res = await axios.get('/auth/roles', { timeout: 8000 })
...
async function saveUser() {
  try {
    if (isEditing.value) {
      ...
      // AQUÍ ESTABA EL ERROR
      await axios.put('/auth/users/' + form.value.id, payload)
...
async function toggleStatus(u) {
  try {
    ...
    // AQUÍ ESTABA EL ERROR
    await axios.put('/auth/users/' + u.id, { status: newStatus })
