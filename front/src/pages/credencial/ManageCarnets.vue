        ...
        <q-card-section>
          ...
          <div class="row">
            ...
            <div class="col-8">
              ...
              <div v-if="selected?.disabled_at || selected?.fecha_deshabilitacion" class="q-mt-xs"><b>Fecha inhabilitación:</b> {{ selected.disabled_at || selected.fecha_deshabilitacion }}</div>
            </div>
          </div>
        </q-card-section>
        <q-card-actions align="right">
...
function mapCredencialToRow(fresh) {
  ...
  const disabled_at = fresh.disabled_at || fresh.fecha_deshabilitacion || null
...
async function loadData() {
  ...
  try {
    ...
    rows.value = data.map(r => {
      ...
      const disabled_at = r.disabled_at || r.fecha_deshabilitacion || null
...
function isRowDisabled(r) {
  if (!r) return false
  return (r.activo === false) || !!(r.disable_reason || r.reason || r.motivo) || !!(r.disabled_at || r.fecha_deshabilitacion)
}
